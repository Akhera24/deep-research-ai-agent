...
# Coverage thresholds
COVERAGE_THRESHOLD = 0.7  # 70% coverage considered adequate
...
class SearchStrategyEngine:
    ...
    Algorithm Flow:
    ┌─────────────────────────────────────────────────────────────┐
    │ 1. Generate Initial Queries (AI-powered)                    │
    │    ↓                                                         │
    │ 2. Execute High-Priority Queries                            │
    │    ↓                                                         │
    │ 3. Extract Entities & Patterns                              │
    │    ↓                                                         │
    │ 4. Update Coverage Metrics                                  │
    │    ↓                                                         │
    │ 5. Generate Follow-up Queries (based on findings)           │
    │    ↓                                                         │
...
2. Progress systematically through all major areas:
   - Biographical (education, background, family)
   - Professional (career, positions, companies)
   - Financial (investments, assets, income)
   - Legal (lawsuits, regulatory issues, compliance)
   - Connections (relationships, networks, affiliations)
   - Behavioral (social media, public statements, activities)
...
[
  ...
  {{
    ...
    def _ai_assisted_refinement(
        ...
        """
        Use AI to generate sophisticated follow-up queries.
        
        Leverages Claude's reasoning to identify non-obvious investigation
        angles based on patterns in the findings.
        """
        # Build concise summary of findings
        findings_summary = self._summarize_findings(findings[:10])
        top_entities = [entity for entity, _ in entities.most_common(5)]
        
        prompt = f"""Based on research findings about {target_name}, suggest {max_queries} follow-up search queries.
...
1. Explore connections between mentioned entities
2. Look for potential red flags or risk indicators
3. Investigate timeline gaps or inconsistencies
4. Uncover hidden structures or beneficial ownership
5. Cross-reference with regulatory or legal databases
  ...
  {{
    ...
    "priority": 4
        ...
        try:
            ...
            # AI-generated queries are typically deeper
    ...
    def update_coverage(
        ...
        Uses diminishing returns formula: harder to reach 100% as you
        approach saturation. Each search provides less incremental value.
        
        Formula: new = current + increment * (1.0 - current)
        
        Args:
            category: Research category to update
            increment: Base increment amount (default 0.15 = 15%)
        
        Example:
            >>> engine.coverage.professional = 0.5
            >>> engine.update_coverage(SearchCategory.PROFESSIONAL, 0.15)
            >>> engine.coverage.professional
            0.575  # 0.5 + 0.15 * (1.0 - 0.5) = 0.575
        """
        current = getattr(self.coverage, category.value)
        
        # Diminishing returns: harder to reach 100%
        new_coverage = min(1.0, current + increment * (1.0 - current))
