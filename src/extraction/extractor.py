...
class Fact:
    ...
    def is_high_confidence(self) -> bool:
        ...
        return self.confidence >= 0.8 and self.verification_count >= 2
...
Extract maximum 30 facts. Start extraction:"""
    ...
    def _post_process_facts(
        ...
        for fact in facts:
            ...
            fact.confidence = (fact.confidence * 0.7) + (avg_reliability * 0.3)
    ...
    def _deduplicate_facts(self, facts: List[Fact]) -> List[Fact]:
        ...
        logger.debug(f"Deduplicated: {len(facts)} → {len(unique)} facts")
        return unique
